...
const Dashboard = () => {
  ...
  const calculateEstimatedCompletionDays = () => {
    ...
    // If no weekly hours yet, assume 40 hours per week (8 hours x 5 days)
    if (!stats.weekly_hours || stats.weekly_hours.length === 0) {
      return Math.ceil(remainingHours / 8);
    }

    // Calculate average daily hours from the last 2 weeks
    const recentWeeks = stats.weekly_hours.slice(0, 2);
    const avgWeeklyHours = recentWeeks.reduce((sum, week) => sum + parseFloat(week.weekly_hours), 0) / recentWeeks.length;
    const avgDailyHours = avgWeeklyHours / 5; // Assume 5 working days per week

    return Math.ceil(remainingHours / avgDailyHours);
  };
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs text-gray-500">
                  Estimated completion: ~{calculateEstimatedCompletionDays()} more working days
                </p>
              </div>
            </div>
