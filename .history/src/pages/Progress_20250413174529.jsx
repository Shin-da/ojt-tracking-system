...
const Progress = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-yellow-50 p-4 border border-yellow-200 rounded-md mb-4" style={{maxWidth: '100%', overflowX: 'auto'}}>
        <h2 className="text-lg font-medium text-yellow-800 mb-2">Debug Information</h2>
        <div className="grid grid-cols-1 gap-2 text-sm">
          <div><strong>API URL:</strong> {API_URL}</div>
          <div><strong>Loading:</strong> {loading ? 'Yes' : 'No'}</div>
          <div><strong>Error:</strong> {error || 'None'}</div>
          <div><strong>CSS Test:</strong> <span className="text-red-600 bg-blue-100 p-1 rounded">This should be red text on blue background</span></div>
          <div><strong>Has Data:</strong> {timeData.heatmap_data?.length > 0 ? 'Yes' : 'No'}</div>
          <div><strong>Data:</strong> <pre className="bg-gray-100 p-2 rounded overflow-auto max-h-32 text-xs">{JSON.stringify({
          ...
          <div><strong>Browser Info:</strong> {window.navigator.userAgent}</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Rendered Time Summary</h2>
