...
const Documents = () => {
    ...
    const fetchDocuments = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`/api/documents/index.php?user_id=1${selectedCategory !== 'all' ? `&category=${selectedCategory}` : ''}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            data.append('user_id', '1'); // Default to 1 for now

            const response = await axios.post('/api/documents/index.php', data, {
                headers: {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await axios.delete(`/api/documents/index.php?id=${id}`);
            if (response.data.success) {
                fetchDocuments();
            } else {
                setError(response.data.message);
                        ...
                        {documents.map((doc) => (
                            <div key={doc.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
                                ...
                                <div className="flex items-center space-x-2">
                                    <a
                                        href={`/api/uploads/documents/${doc.file_path}`}
                                        target="_blank"
